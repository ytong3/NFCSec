...
public class Cpabe {
	...
	public void enc(String pubfile, String policy, String inputfile,
			String encfile, int pk_source) throws Exception {
		BswabePub pub;
		BswabeCph cph;
		BswabeCphKey keyCph;
		byte[] plt;
		...
		/* get BswabePub from pubfile */
		if(pk_source==Common.FROM_ASSET)
			pub_byte = Common.readFileFromAsset(pubfile);
		else
			pub_byte = Common.suckFile(pubfile);
	...
	public void dec(String pubfile, String prvfile, String encfile,
			String decfile, int keySource) throws Exception {
		byte[] aesBuf, cphBuf;
		byte[] plt;
		byte[] prv_byte;
		byte[] pub_byte;
		...
		/* get BswabePub from pubfile */
		if(keySource==Common.FROM_ASSET){
			pub_byte = Common.readFileFromAsset(pubfile);
			prv_byte = Common.readFileFromAsset(prvfile);
		}
		else{
			pub_byte = Common.suckFile(pubfile);
